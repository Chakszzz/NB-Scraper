...
export class WeatherMaster {
  ...
  constructor(options = {}) {
    ...
    this.headers = {
      "User-Agent":
        "Mozilla/5.0 (Linux; Android 11; 220333QAG Build/RKQ1.211001.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/137.0.7151.89 Mobile Safari/537.36",
    ...
    try {
      ...
      const res = await axios.get(url, {
        params: {
          ...
          daily:
            "weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,daylight_duration,uv_index_max,precipitation_sum,daylight_duration,precipitation_probability_max,precipitation_hours,wind_speed_10m_max,wind_gusts_10m_max",
  ...
  async fetchOpenMeteoData(params) {
    try {
      const url = "https://api.open-meteo.com/v1/forecast";
      const res = await axios.get(url, {
        params: {
  ...
  async getOpenMeteoHourly() {
    return this.fetchOpenMeteoData({
      hourly:
        "relative_humidity_2m,pressure_msl,cloud_cover,temperature_2m,dew_point_2m,apparent_temperature,precipitation_probability,precipitation,weather_code,visibility,uv_index",
      forecast_days: 14,
    });
  }
}
